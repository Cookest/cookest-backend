//! Cookest App API — Rust + Actix-Web + SeaORM + PostgreSQL
//!
//! Features:
...
async fn main() -> std::io::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::new(
            std::env::var("RUST_LOG").unwrap_or_else(|_| "info,cookest_app_api=debug".to_string()),
        ))
        .with(tracing_subscriber::fmt::layer())
        .init();

    tracing::info!("Starting Cookest App API");
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            // CORS
            .wrap(cors)
            // Security headers (outermost — ensures headers on all responses incl. 429)
